...
int countLessNArr(int num, double less_num);
...
int main()
{
	...
	string answer = "Minimum element number: " + to_string(minIArr(n) + 1)
		+ "\nSum of vector elements located between the first and second"
		" negative elements: ";

	int firstNegI = findNegIArr(0, n - 1, 1);
	int secondNegI = findNegIArr(firstNegI + 1, n - 1, 1);
	if (countLessNArr(n, 0) > 1)
	{
		answer += doubleToStr(sumNumArr(firstNegI + 1, secondNegI - 1, 1));
	}
	else
	{
		answer += "none";
